...
function renderHistory(report){
  ...
  el('history-state').textContent=report.stale?'Provisional P&L: snapshots do not include all imported activity. Values are as of the dates shown below.':report.provisional?'Provisional report: some history or valuation data is incomplete or requires review. N/A means a reliable value is unavailable.':'';
