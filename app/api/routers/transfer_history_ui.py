...
.page-heading,main{max-width:1200px;margin:auto;padding:24px}.page-heading{display:flex;justify-content:space-between;align-items:center;gap:16px}h1{margin:0;font-size:24px}.muted{color:var(--muted)}
...
@media(max-width:650px){.page-heading{align-items:flex-start;flex-direction:column}.page-heading,main{padding:16px}}
</style></head><body><div class="page-heading"><div><h1>Transfer history</h1><div class="muted">All deposits and withdrawals · Newest first · Original currency</div></div></div>
...
for(const id of ['previous-page','next-page'])el(id).disabled=true;
try{
let x;
while(true){
...
finally{loading=false;el('previous-page').disabled=offset===0;el('next-page').disabled=!hasMore}
}
