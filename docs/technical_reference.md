...
Generated API documentation is available at `/docs` on the running application.
...
- `IBKR_FLEX_RETRY_ATTEMPTS` (default `10`)
- `IBKR_FLEX_BACKOFF_BASE_SECONDS` (default `10.0`)
- `IBKR_FLEX_BACKOFF_MAX_SECONDS` (default `45.0`)
...
PostgreSQL before using that cleanup-capable command; see [docs/operations.md](../docs/operations.md).
...
- `POST /corporate-actions/cases/{case_id}/resolution/preview` accepts a `treatment`
  (`security_transfer` or `distribution`) and broker-evidence `note`. Distributions
  also require an explicit nonnegative `cost_basis` in the credited security's currency.
- `POST /corporate-actions/cases/{case_id}/resolution/apply` accepts those fields plus
  the returned `preview_token`, and atomically applies an unchanged preview.
...
The review queue counts only available decisions. Paired identifier changes show both
...
retention, PITR, and incident recovery are documented in [deploy/systemd/README.md](../deploy/systemd/README.md) and
[docs/operations.md](../docs/operations.md).
