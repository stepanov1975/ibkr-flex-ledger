# Accounting, API, and reliability contracts

Baseline approved: 2026-02-14
Documentation reviewed: 2026-09-12
Status: Maintained contracts and explicitly identified operational targets

## Purpose

This document defines accounting, ingestion, API, export, and reliability contracts.
The [technical reference](technical_reference.md) describes their runtime integration;
the [operations guide](operations.md) distinguishes installed procedures from recovery targets.
The [original freeze sheet](archive/MVP_spec_freeze.md) preserves approval history.
...
- Account scope: strictly single IBKR account.
- Single-account API contract: `account_id` remains internal-only in API payloads; backend data-access layers use one fixed configured account context.
- Base reporting currency: `USD`, fixed at initial setup and immutable.
- Time policy: persist timestamps in UTC; use `Asia/Jerusalem` for timestamp display and timestamp-derived business boundaries. Explicit Flex statement dates retain their reported calendar date. UI date-only values use zero-padded `dd/mm/yy`; UI timestamps use zero-padded `dd/mm/yy hh:mm` with 24-hour time. API payloads retain their ISO date and UTC timestamp contracts.
...
- Authentication hardening for proxy headers/trust assumptions: not implemented; access is restricted to a trusted network or an operator-managed reverse proxy.
- Reverse-proxy identity header contract validation in application code: not implemented; access is restricted to a trusted network or an operator-managed reverse proxy.
...
Broker-authority rule: completed `OpenPositions` artifacts follow the broker-authoritative
reconciliation design in [broker-position reconciliation design](superpowers/specs/2026-08-21-broker-position-reconciliation-design.md).
...
status acknowledgement alone cannot clear uncertainty. Unsupported parent-basis allocation,
incomplete transfers, and cash matching remain provisional. See the
[corporate-action workflow](technical_reference.md#corporate-action-review-workflow-task-8)
for eligibility, source binding, invalidation, and replay rules.
...
- Affected outputs remain provisional until accounting is resolved; status-only acknowledgement does not resolve it.
...
These are policy targets. The current operations endpoint evaluates success rate and
duration over 30 days; the original 14-day p95 target below remains a policy target,
not a claim that the endpoint implements a separate 14-day window.

Goal: define measurable reliability targets and alert thresholds.
...
The table retains the approved retention and restore targets. The installed job archives
old ingestion-run diagnostics to checksum-verified compressed JSONL before clearing their
payloads; it does not implement separate monthly reprocess or snapshot archive pipelines.
See [diagnostic retention](operations.md#diagnostic-retention) for the current procedure.
...
Goal: freeze section-level ingestion requirements so supported features are deterministic and schema-drift behavior is explicit.
...
### 8.4 Other optional sections

`CashReport` supplies broker cash balances and portfolio totals without being a hard-required
section. Other sections are retained in immutable raw artifacts and extracted raw rows;
canonical mapping applies only to explicitly supported section types.
...
Paginated resource-list response envelope:
- `items`: array
...
## Original decision sign-off (2026-02-14)

| Section | Status (`open`/`approved`) | Decision Owner | Decision Date | Notes |
|---|---|---|---|---|
| Natural keys | approved | Product + Engineering | 2026-02-14 | Reference-informed initial baseline frozen |
...
| Single-account API contract (`account_id`) | approved | Product + Engineering | 2026-02-14 | `account_id` internal-only in initial payloads; fixed configured account context in backend |
